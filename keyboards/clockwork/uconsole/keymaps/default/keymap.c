...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [_DEF] = LAYOUT(
        ...
        KC_LSFT,         KC_Z,      KC_X,       KC_C,       KC_V,      KC_B,        KC_N,       KC_M,       KC_COMM,    KC_DOT,     KC_RSFT,
            OSL(_FN),       KC_LCTL,    KC_LALT,                            KC_SPC,                     KC_RALT,    KC_RCTL,    TT(_FN)
    ),
    [_FN] = LAYOUT(
        ...
        _______,    _______,    _______,    _______,    _______,    _______,    KC_HOME,    KC_END,    KC_PGDN,     _______,    _______,    _______,
        _______,         _______,   _______,    _______,    _______,    QK_BOOT,    _______,    _______,    KC_BRID,    KC_BRIU,    _______,
...
const key_override_t shift_vol_up_override = ko_make_basic(MOD_MASK_SHIFT, KC_VOLU, KC_VOLD);

// clang-format off
const key_override_t **key_overrides = (const key_override_t *[]){
    &shift_vol_up_override,
    NULL,
};
// clang-format on
#endif
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    if (record->event.pressed == 1) {
        dprintf("KC 0x%04X pressed, C=%02u R=%02u L=0x%02X P=%u T=%5u C=%u I=%u\n", keycode, record->event.key.col, record->event.key.row, layer_state, record->event.pressed, record->event.time, record->tap.count, record->tap.interrupted);
    } else {
        dprintf("KC 0x%04X release, C=%02u R=%02u L=0x%02X P=%u T=%5u C=%u I=%u\n", keycode, record->event.key.col, record->event.key.row, layer_state, record->event.pressed, record->event.time, record->tap.count, record->tap.interrupted);
    }
#endif
    return true;
}
